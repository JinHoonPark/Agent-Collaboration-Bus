...
아래는 이 스펙이 딛고 선 전제 중 아직 확인되지 않은 것들이다. **사양의 일부다** —
구현 계획은 각 항목을 해당 단계의 선행 확인 작업으로 포함해야 한다.

| 전제 | 관련 절 | 확인 시점 |
|---|---|---|
| **`Mcp-Session-Id` 가 한 세션의 여러 툴 호출에 걸쳐 유지된다** | 6.1 | **M1. 코드 작성 전** |
| **에이전트 두 개가 요청·응답 왕복을 완주한다** | 전체 | **M1 최우선** |
| 수신 에이전트가 자기 사용자에게 보고만 하지 않고 `acb_reply` 를 호출한다 | 6.2 | M1 왕복 테스트 |
| 훅 주입 지시가 파일 조작이 아니라 **MCP 툴 호출**(`acb_inbox`)로도 동작한다 | 7.2, 7.4 | M1 통합 테스트 |
| Streamable HTTP 전송에서 elicitation 이 동작한다 | 7.2 | M1 브로커 기동 시 |
| HTTP 전송에서 "세션 1개 = 연결 1개"가 성립한다 | 6.1 | M1 |
| 모델이 `acb_wait` 45초 반복 폴링을 포기하지 않고 견딘다 | 6.2 | M2 |
| 다건 동시 배달과 `additionalContext` 2,500 토큰 초과 시의 축약 동작 | 7.2 | M2 |
| 툴 호출 **진행 중**에도 elicitation 이 렌더링된다 | 7.2 | M3 |
| 자동 거부와 사람 거부를 가르는 응답 지연 임계값 | 7.2 | M3 |
| Codex CLI 의 마켓플레이스 배포 경로 | 9.1 | M6 |

확인 실패 시의 폴백은 해당 절에 함께 적혀 있다. `Mcp-Session-Id` 는 툴 인자 방식으로,
elicitation 은 OS 알림으로, Codex 플러그인은 수동 등록으로 각각 되돌린다. 폴백이 적혀
있지 않은 항목은 **확인 결과에 따라 설계 판단이 필요하다는 뜻이다.**

특히 위 두 항목은 성격이 다르다. 나머지는 폴백이 있지만, **에이전트 간 왕복 완주는
폴백이 없다.** 수신 에이전트가 메시지를 처리하고도 응답하지 않는다면 그것은 배관
문제가 아니라 모델 행동 문제이고, 코드로 보장되지 않는다.
