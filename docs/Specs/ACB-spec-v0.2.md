...
- 훅 스크립트는 **사용자가 직접 설치**하고, 브로커가 원격에서 덮어쓰지 않는다.
  ACB 가 훅 스크립트를 자동 배포·자동 업데이트하면 브로커가 모든 참여 워크스페이스의
  코드 실행 권한을 갖게 된다. v1 에서는 하지 않는다.
- 훅 스크립트는 브로커에서 받은 텍스트를 **주입할 뿐 실행하지 않는다.**
  주입 문구는 10.2 의 래핑 규칙을 그대로 따른다.
- 훅 스크립트 경로는 워크스페이스 내부에 두고, 다른 참여자가 쓰기 가능한 공유
  위치에 두지 않는다.
